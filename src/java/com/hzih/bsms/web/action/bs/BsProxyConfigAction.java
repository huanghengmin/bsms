...
import com.hzih.bsms.dao.HttpsDao;
...
import com.hzih.bsms.web.action.bsps.UploadBspsConfig;
import com.hzih.bsms.web.action.proxy_address.BsProxyConfigXml;
...
//    private String bsms_nginx_path = StringContext.nginxPath + "/bsPx.conf";
//    private String bsms_tcp_path = StringContext.nginxPath + "/tcp.conf";
//    private String bsms_nginx_path =StringContext.nginxConfigPath+ "/bsMs.conf";// StringContext.nginxConfigPath+ "/bsPx.conf";
    private String bsms_tcp_path =StringContext.nginxConfigPath+ "/tcp.conf"; //StringContext.nginxConfigPath + "/tcp.conf";
//    private String bsps_nginx_path =StringContext.bspsConfigPath+ "/bsPx.conf";// StringContext.nginxConfigPath+ "/bsPx.conf";
//    private String bsps_tcp_config =StringContext.bspsConfigPath+ "/bsPsTcp.conf"; //StringContext.nginxConfigPath + "/tcp.conf";
//    private String bsps_t_http_config = StringContext.nginxConfigPath+"/thttp.conf";
//    private String StringContext.bsps_t_http_config = StringContext.bspsConfigPath + "/bsTHttp.conf";//StringContext.squidConfigPath+"/config.conf";
...
//                    }
                ...
                try {
                    ...
                    List<Https> t_httpList =httpsDao.findAllTHttp();
                    try{
                        convertBSMSNginxTcpConf(tcpList,t_httpList, bsms_tcp_path);
                        convertBSPSNginxTcpConf(tcpList,StringContext.bsps_tcp_config);
                    }catch (Exception e){
                ...
                try {
                    ...
                    try{
                        convertBSMSNginxConf(httpList, bsms_nginx_path);
                        convertBSPSNginxConf(httpList,StringContext.bsps_nginx_config);
                    }catch (Exception e){
...
//                https.setProxy_port(proxyport);
                try {
                    ...
                    List<Https> tcp_List =httpsDao.findAllTcp();
                    try{
                        convertBSPSNginxTHttpConf(httpList, StringContext.bsps_t_http_config);
                        convertBSMSNginxTcpConf(tcp_List,httpList,bsms_tcp_path);
                    }catch (Exception e){
...
//            try {
//                BsMsConvertConfig.convert(config_path,bsms_nginx_path);
//            } catch (IOException e) {
...
//            BsMsConvertConfig.convert(config_path,bsms_nginx_path);
       /* if(https.getProtocol().equals("https")){
            ...
            List<Https> t_httpList =httpsDao.findAllTHttp();
            try{
                convertBSMSNginxTcpConf(httpList,t_httpList, bsms_tcp_path);
                convertBSPSNginxTcpConf(httpList,StringContext.bsps_tcp_config);
            }catch (Exception e){
                logger.info("转换nginx tcp 配置文件不成功!");
            }
            ...
            List<Https> httpList =httpsDao.findAllTcp();
            try{
//                convertBSPSNginxTHttpConf(t_httpList, StringContext.bsps_t_http_config);
                convertBSMSNginxTcpConf(httpList,t_httpList, bsms_tcp_path);
            }catch (Exception e){
                logger.info("转换nginx配置文件不成功!");
            }
           ...
           /* if("https".equals(oldProtocol)){
                ...
                if(protocol.equals("http")){
                    ...
                    try {
                        ...
                        List<Https> httpList =httpsDao.findAllTcp();
                        try{
                            convertBSMSNginxTcpConf(httpList,bsms_tcp_path);
                        }catch (Exception e){
                            logger.info("转换nginx tcp 配置文件不成功!");
                        }
                        save_flag = true;
...
//                    https.setProxy_port(proxyport);
                    try {
                        ...
                        List<Https> httpList =httpsDao.findAllTHttp();
                        try{
                            convertBSPSNginxTHttpConf(httpList,StringContext.bsps_t_http_configsps_t_http_configsps_t_http_config);
                        }catch (Exception e){
                            logger.info("转换nginx 透明代理 配置文件不成功!");
                        }
                        save_flag = true;
                ...
                if(protocol.equals("http")){
                    ...
                    try {
                        ...
                        try{
                            convertBSMSNginxTcpConf(httpList,t_httpList, bsms_tcp_path);
                            convertBSPSNginxTcpConf(httpList,StringContext.bsps_tcp_config);
//                            convertBSMSNginxConf(http_List, bsms_nginx_path);
//                            convertBSPSNginxConf(http_List,StringContext.bsps_nginx_config);
                        }catch (Exception e){
                            logger.info("转换nginx tcp 配置文件不成功!");
                        }
                        save_flag = true;
...
//                    https.setProxy_port(proxyport);
                    try {
                        ...
                        List<Https> tcp_List =httpsDao.findAllTcp();
                        try{
                            convertBSMSNginxTcpConf(tcp_List,http_List, bsms_tcp_path);
                            convertBSPSNginxTHttpConf(http_List, StringContext.bsps_t_http_config);
                            convertBSPSNginxConf(httpList,StringContext.bsps_nginx_config);
                            convertBSMSNginxConf(httpList, bsms_nginx_path);
                        }catch (Exception e){
                            logger.info("转换nginx 透明代理 配置文件不成功!");
...
//                                https_modify_process.start();
               ...
               *//* if(protocol.equals("http")){
                    ...
                    try {
                        ...
                        try{
//                            convertBSMSNginxConf(httpList, bsms_nginx_path);
//                            convertBSPSNginxConf(httpList,StringContext.bsps_tcp_config);
//                            convertBSPSNginxTHttpConf(http_List, StringContext.bsps_t_http_config);
                            convertBSMSNginxTcpConf(tcp_List,http_List,bsms_tcp_path);
                        }catch (Exception e){
                            logger.info("转换nginx配置文件不成功!");
...
//                    https.setProxy_port(proxyport);
                        try {
                            ...
                            List<Https> httpList =httpsDao.findAllTHttp();
                            try{
                                convertBSPSNginxTHttpConf(httpList,StringContext.bsps_t_http_configsps_t_http_config);
                            }catch (Exception e){
                                logger.info("转换nginx 透明代理 配置文件不成功!");
                            }
                            save_flag = true;
...
//                                https_modify_process.start();
    ...
    //构建bsms http代理文件
   /* public void convertBSMSNginxConf(List<Https> httpList,String outputFilePath)throws IOException{
...
//                String proxyPort = https.getProxy_port();//pro_element.attributeValue("port");
                String  proxyIp = BsProxyConfigXml.getAttribute(BsProxyConfigXml.bs_proxy_ip);
//                String proxyPort = https.getProxy_port();
                ...
                //            sb.append("#      break;\n");
                //            sb.append("#      }\n");
                sb.append("proxy_pass "+managerProtocol+"://"+proxyIp+":"+managerPort+";\n" );
   ...
   //构建bsms tcp代理文件
    private void convertBSMSNginxTcpConf(List<Https> tcpList,List<Https> t_httpList, String tcp_path)throws IOException{
//        int maxConnect =0 ;
        StringBuilder sb = new StringBuilder("");
        int count =0 ;
...
//                String proxyPort = https.getProxy_port();
                //加入语句
                sb.append("upstream cluster"+count+"{").append("\n");
                sb.append("server "+BsProxyConfigXml.getAttribute(BsProxyConfigXml.bs_proxy_ip)+":"+managerPort+";").append("\n");
                sb.append("check interval=3000 rise=2 fall=5 timeout=1000;").append("\n");
                sb.append("}").append("\n");

                sb.append("server {").append("\n");
                sb.append("listen "+managerIp+":"+managerPort+";").append("\n");
                sb.append("proxy_pass cluster"+count+";").append("\n");
                sb.append("}").append("\n");
                count++;
            }
        }
        if(null!=t_httpList&&t_httpList.size()>0){
            for (int i=0;i<t_httpList.size();i++){
                Https https = t_httpList.get(i);
                String managerIp =https.getManager_ip();
                String managerPort = https.getManager_port();
//                String managerProtocol = https.getProtocol();
//                String proxyIp = https.getProxy_ip();
//                String proxyPort = https.getProxy_port();
    ...
    //构建bsps http
   /* public void convertBSPSNginxConf(List<Https> httpList,String outputFilePath)throws IOException{
//       int  maxConnect = License.getMaxConnect();
        int maxConnect =0 ;
        StringBuilder sb = new StringBuilder("");
        if(httpList!=null&&httpList.size()>0){
...
//                Element manager = managers.get(i);
                ...
                String managerIp =BsProxyConfigXml.getAttribute(BsProxyConfigXml.bs_proxy_ip); //manager.attributeValue("ip");
                String managerPort = https.getManager_port();
...
//                "80/bsps/UploadConfigFiles_upload_tHttp_config.action";
        UploadBspsConfig.upload_tcp_config(http_url,outputFilePath);
    }*/
   //构建bsps tcp 代理文件
   private void convertBSPSNginxTcpConf(List<Https> tcpList, String tcp_path)throws IOException{
//        int maxConnect =0 ;
      StringBuilder sb = new StringBuilder("");
      if(tcpList!=null&&tcpList.size()>0){
          for (int i=0;i<tcpList.size();i++){
...
//                String managerProtocol = https.getProtocol();
              ...
              sb.append("server {").append("\n");
              sb.append("listen "+BsProxyConfigXml.getAttribute(BsProxyConfigXml.bs_proxy_ip)+":"+managerPort+";").append("\n");
      ...
      out.close();
       ...
       final String tcp_url  = "http://"+ BsProxyConfigXml.getAttribute(BsProxyConfigXml.bs_proxy_ip)+":"+
               "80/UploadConfigFiles_upload_tcp_config.action";
//        String http_url  = "http://"+ BsProxyConfigXml.getAttribute(BsProxyConfigXml.bs_proxy_ip)+":"+
//                "80/bsps/UploadConfigFiles_upload_http_config.action";
//        String t_http_url  = "http://"+ BsProxyConfigXml.getAttribute(BsProxyConfigXml.bs_proxy_ip)+":"+
//                "80/bsps/UploadConfigFiles_upload_tHttp_config.action";
       UploadBspsConfig.upload_tcp_config(tcp_url,tcp_path);
  }
    //构建透明代理文件
   /* private void convertBSPSNginxTHttpConf(List<Https> tcpList, String thttpPath)throws IOException{
//        int maxConnect =0 ;
        StringBuilder sb = new StringBuilder("");
        if(tcpList!=null&&tcpList.size()>0){
            for (int i=0;i<tcpList.size();i++){
...
//                sb.append("}").append("\n");
//                sb.append("}").append("\n");
                sb.append("http_port "+BsProxyConfigXml.getAttribute(BsProxyConfigXml.bs_proxy_ip)+":"+managerPort).append("\n");
...
//                "80/bsps/UploadConfigFiles_upload_http_config.action";
        String t_http_url  = "http://"+ BsProxyConfigXml.getAttribute(BsProxyConfigXml.bs_proxy_ip)+":"+
                "80/bsps/UploadConfigFiles_upload_tHttp_config.action";
        UploadBspsConfig.upload_tcp_config(t_http_url,thttpPath);
    }*/
    //热重启服务
    public void reconfigure_server(){
        Proc proc = new Proc();
//        proc.exec("sh "+StringContext.systemPath+"/bsshell/reconfigure.sh");
