...
import com.hzih.bsms.dao.RoleManageDao;
import com.hzih.bsms.domain.RoleManage;
...
public class RoleManageDaoImpl extends HibernateDaoSupport implements RoleManageDao{
    ...
    public ArrayList<RoleManage> findRoleManages() {
        String hql = "select t from com.hzih.bsms.domain.RoleManage t";
    ...
    public ArrayList<RoleManage> findRoleManages(final int start,final int limit) {
        final String hql = "select t from com.hzih.bsms.domain.RoleManage t order by t.id asc";
    ...
    public ArrayList<RoleManage> findRoleManages(String rolename) {
        String hql = String.format("select t from com.hzih.bsms.domain.RoleManage t where t.rolename like'%%%s%%'", rolename);
        ArrayList<RoleManage> list = (ArrayList<RoleManage>) this.getHibernateTemplate().find(hql);
        return list;
    }
    public ArrayList<RoleManage> findRoleManages(String rolename,final int start,final int limit) {
        final String hql = String.format("select t from com.hzih.bsms.domain.RoleManage t where t.rolename like'%%%s%%' order by t.id asc", rolename);
        List list = this.getHibernateTemplate().find(hql);
        if( list.size()!=0 ){
            list = getHibernateTemplate().executeFind(
                    new HibernateCallback() {
    ...
    public ArrayList<RoleManage> findRoleManagesById(int id) {
        String hql = "select t from com.hzih.bsms.domain.RoleManage t where t.id=?";
    ...
    public ArrayList<RoleManage> findRoleManagesByRolename(String rolename) {
        String hql = "select t from com.hzih.bsms.domain.RoleManage t where t.rolename=?";
