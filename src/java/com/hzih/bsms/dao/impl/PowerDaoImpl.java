...
import com.hzih.bsms.dao.PowerDao;
import com.hzih.bsms.domain.Power;
...
public class PowerDaoImpl extends HibernateDaoSupport implements PowerDao {
    ...
    public ArrayList<Power> findPowers() {
        String hql = "select t from com.hzih.bsms.domain.Power t";
    ...
    public ArrayList<Power> findPowers(final int start,final int limit) {
        final String hql = "select t from com.hzih.bsms.domain.Power t";
    ...
    public ArrayList<Power> findPowersById(int id) {
        String hql = "select t from com.hzih.bsms.domain.Power t where t.id=?";
    ...
    public ArrayList<Power> findPowersByIds(String ids) {
        String hql = "select t from com.hzih.bsms.domain.Power t where t.id in ("+ids+")";
        ArrayList<Power> list = (ArrayList<Power>) this.getHibernateTemplate().find(hql);
        return list;
    }
    ...
    public ArrayList<Power> findPowersByRoleName(String rolename) {
        String hql = "select t from com.hzih.bsms.domain.Power t where t.rolename=?";
    ...
    public ArrayList<Power> findPowersByResourceName(String resourcename) {
        String hql = "select t from com.hzih.bsms.domain.Power t where t.resourcename=?";
    ...
    public ArrayList<Power> findPowersByTwoLikeNames(String rolename, String resourcename) {
        String hql = "select t from com.hzih.bsms.domain.Power t where t.rolename like '%"+rolename+"%' and t.resourcename like '%"+resourcename+"%'";
        ArrayList<Power> list = (ArrayList<Power>) this.getHibernateTemplate().find(hql);
        return list;
    }
    @Override
    public ArrayList<Power> findPowersByTwoLikeNames(String rolename, String resourcename,final int start,final int limit) {
        final String hql = "select t from com.hzih.bsms.domain.Power t where t.rolename like '%"+rolename+"%' and t.resourcename like '%"+resourcename+"%'";
        List list = this.getHibernateTemplate().find(hql);
        if( list.size()!=0 ){
            list = getHibernateTemplate().executeFind(
                    new HibernateCallback() {
